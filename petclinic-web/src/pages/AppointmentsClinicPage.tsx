...
import { appointmentsClinicApi, petsApi, vetsApi } from "../api/resources";
import { useResource } from "../hooks/useResource";
import type { AppointmentClinic, CreateAppointmentClinicDto, Pet, UpdateAppointmentClinicDto, Vet } from "../types";
...
const emptyForm: CreateAppointmentClinicDto = { vetId: "", petId: "", dateAppointment: "", notes: "" };
...
export default function AppointmentsClinicPage() {
  const { items, loading, error, reload, create, update, remove } = useResource(appointmentsClinicApi);
  ...
  const [editing, setEditing] = useState<AppointmentClinic | null>(null);
  const [form, setForm] = useState<CreateAppointmentClinicDto>(emptyForm);
  const [submitting, setSubmitting] = useState(false);
  const [formError, setFormError] = useState<string | null>(null);
  const [toRemove, setToRemove] = useState<AppointmentClinic | null>(null);
  ...
  const openEdit = (appt: AppointmentClinic) => {
    setEditing(appt);
    setForm({
      vetId: appt.vetId,
      petId: appt.petId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...form,
        dateAppointment: new Date(form.dateAppointment).toISOString(),
        notes: form.notes?.trim() ? form.notes.trim() : null,
      };
      if (editing) {
        const dto: UpdateAppointmentClinicDto = { id: editing.id, ...payload };
        await update(dto);
      } else {
  ...
  return (
    <div>
      <PageHeader
        title="Consultas (Clínica)"
        description="Agendamentos da Clínica, com observações do veterinário."
      ...
      {!loading && !error && items.length === 0 && (
        <EmptyState message="Nenhuma consulta agendada na clínica ainda." />
      )}
            ...
            <div key={appt.id} className="record-card" style={{ ["--strip-color" as string]: "#3F6FD1" }}>
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  {appt.notes && (
                    <p className="text-xs text-muted mt-2 line-clamp-2 italic">"{appt.notes}"</p>
                  )}
                </div>
      ...
      <Modal title={editing ? "Editar consulta" : "Nova consulta"} open={modalOpen} onClose={() => setModalOpen(false)}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <TextareaField label="Observações" htmlFor="notes" value={form.notes ?? ""} placeholder="Opcional"
